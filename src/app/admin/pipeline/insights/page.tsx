...
function getUrgencyIcon(urgency: string) {
  const icons: Record<string, { color: string; label: string }> = {
    ...
    trending: { color: "text-red-400", label: "Trending" },
  };
...
function getStatusConfig(status: string) {
  const config: Record<string, { bg: string; text: string; label: string }> = {
    ...
    used: { bg: "bg-purple-500/15", text: "text-purple-400", label: "Used" },
  };
    ...
    return (
      <div className="animate-pulse space-y-6">
        ...
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-28 bg-[#18181b] rounded-xl" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-[#71717a] text-sm mt-1">AI-discovered content opportunities and trends</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          { label: "Used", value: stats.used, color: "text-purple-400" },
        ].map((stat) => (
      ...
      <div className="space-y-3">
        {insights.length === 0 ? (
          <div className="bg-[#18181b] border border-[#27272a] rounded-xl p-12 text-center">
            ...
            <p className="text-[#71717a] text-sm mb-4">Run a trend scan to discover content opportunities</p>
            <button
              onClick={() => handleAction("scan", "")}
              disabled={actionLoading !== null}
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      {insight.journal_pillars && (
                        <span className="px-2.5 py-1 rounded-md text-xs bg-[#27272a] text-[#a1a1aa]">
                    ...
                    {/* Title & Summary */}
                    <h3 className="text-base font-medium text-white mb-2 line-clamp-1">{insight.title}</h3>
                    <p className="text-[#a1a1aa] text-sm mb-3 line-clamp-2">{insight.summary}</p>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {insight.target_keywords.length > 4 && (
                        <span className="text-xs text-[#52525b]">+{insight.target_keywords.length - 4} more</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    ...
                    {insight.status === "rejected" && (
                      <span className="text-xs text-[#71717a] text-center">Rejected</span>
                    )}
                    {insight.status === "used" && (
                      <span className="text-xs text-purple-400 text-center">Brief created</span>
                    )}
                  </div>
                </div>
                ...
                {insight.key_points.length > 0 && insight.status === "new" && (
                  <details className="mt-4 pt-4 border-t border-[#27272a]">
                    <summary className="text-xs text-[#71717a] cursor-pointer hover:text-white transition-colors">
      ...
      {selectedInsight && !showRejectModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#18181b] border border-[#27272a] rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {selectedInsight.journal_pillars && (
                      <span className="px-2 py-0.5 rounded text-xs bg-[#27272a] text-[#a1a1aa]">
                        {selectedInsight.journal_pillars.name}
                      </span>
                    )}
                  </div>
                  <h2 className="text-xl font-semibold text-white">{selectedInsight.title}</h2>
                </div>
                <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div>
                <h3 className="text-sm font-medium text-[#71717a] mb-2">Summary</h3>
                <p className="text-[#e4e4e7]">{selectedInsight.summary}</p>
              </div>

              <div>
                <h3 className="text-sm font-medium text-[#71717a] mb-2">Key Points</h3>
              ...
              <div>
                <h3 className="text-sm font-medium text-[#71717a] mb-2">Target Keywords</h3>
              ...
              <div className="grid grid-cols-3 gap-4 pt-4 border-t border-[#27272a]">
                <div>
                  <p className="text-xs text-[#71717a] mb-1">Relevance</p>
                  <p className="text-lg font-semibold text-white">{Math.round((selectedInsight.relevance_score || 0) * 100)}%</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold text-white">{selectedInsight.source_type}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 border-t border-[#27272a] flex justify-end gap-3 bg-[#18181b]">
              ...
              <button
                ...
                className="px-4 py-2.5 bg-red-500/15 text-red-400 font-medium rounded-lg hover:bg-red-500/25 transition-colors"
              >
                Reject
              </button>
              <button
      ...
      {showRejectModal && selectedInsight && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#18181b] border border-[#27272a] rounded-2xl max-w-md w-full">
            <div className="p-6 border-b border-[#27272a]">
              <h2 className="text-lg font-semibold text-white">Reject Insight</h2>
              <p className="text-sm text-[#71717a] mt-1">Why is this insight not suitable?</p>
            </div>
            <div className="p-6">
              <textarea
                value={rejectReason}
